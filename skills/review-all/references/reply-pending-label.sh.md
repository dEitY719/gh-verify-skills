...
train 은 답변·`/simplify` 수정이 아직 날아오는 중인 PR 을 머지한다 (PR #1522).
...
`Skill(devx:schedule, ...)` 예약은 이 블록의 성공 여부와 무관하게 **반드시**
...
GH_HOST="$TARGET_HOST" gh label create "reply-pending" --repo "$TARGET_REPO" \
    --color "fbca04" \
    --description "지연된 리뷰 답변 패스가 아직 남아 있는 PR — merge-train 이 건너뛴다 (#1524)" \
    >/dev/null 2>&1 || true
