...
`gh-issue:issue-create` 에 넘긴다. 이 스킬은 **게이트만** 책임진다.
...
후보 발견
  ...
  -> 4. 대상 레포 해석 후 출력
  -> 5. Skill(gh-issue:issue-create, "--assignee @me") 호출  -> safe 라벨 -> 보드 카드 동기화
```
...
## 5. 등록은 `gh-issue:issue-create` 에 위임한다

발견 1건마다 `Skill(gh-issue:issue-create, "--assignee @me")` 를 호출한다.
본문 골격 · 라벨 SSOT 판정 · ai-metrics 푸터는 **그 스킬이 SSOT** 다.

**여기서 템플릿을 재기술하지 않는다** — 재기술하면 그쪽이 바뀔 때 두 곳이 어긋난다.
원 §8 의 "TL;DR / 증상 / 재현 / 근본 원인 / …" 나열은 `gh-issue:issue-create` 의
...
| 플래그 | 동작 |
|---|---|
| (없음) | 게이트 통과분을 `gh-issue:issue-create` 로 등록 |
